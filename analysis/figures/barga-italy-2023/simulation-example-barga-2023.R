...
p_a <-
  ggplot(d, aes(x, y)) +
  coord_equal() +
  geom_tile(data = HABITAT_tbl, fill = 'transparent', color = 'black')+
  geom_path(lwd = 1, linejoin = 'round', color = 'grey') +
  geom_point(aes(0, 0), pch = 18, size = 6) +
  geom_point(aes(0, 0), pch = 18, size = 4, color = 'yellow')

# b) encounters (green dots)
p_b <-
  p_a +
  geom_point(data = filter(d, new_cell), size = 2, color = 'darkgreen') +
  geom_point(aes(0, 0), pch = 18, size = 6) +
  geom_point(aes(0, 0), pch = 18, size = 4, color = 'yellow')
...
p_c <-
  ...
  geom_point(data = filter(track, new_cell), size = 2, color = 'darkgreen') +
  geom_point(aes(0, 0), pch = 18, size = 6) +
  geom_point(aes(0, 0), pch = 18, size = 4, color = 'yellow') +
...
p_e <-
  ...
  coord_equal() +
  geom_tile(data = HABITAT_tbl, fill = 'transparent', color = 'black') +
  geom_polygon(aes(long, lat), color = pal[3], fill = pal[3], alpha = 0.3,
...
p <-
  plot_grid(p_a, p_b, p_c, p_d, p_e, leg, nrow = 1, label_size = 22,
            labels = c('a.', 'b.', 'c.', 'd.', 'e.', ''),
            label_x = -0.04, label_y = 0.95, rel_widths = c(rep(1, 5), 0.3))

ggsave('figures/2023-GRS-GRC-barga-italy/simulation-example.png', plot = p,
       width = 40, height = 7, bg = 'white', units = 'cm')
