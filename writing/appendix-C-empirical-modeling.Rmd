...
print('H:/GitHub/hr-resource-stoch/writing') # for anonymity in peer review
```
...
Next, we can estimate the mean and variance in NDVI for each 7-day period by taking the average for the GPS locations within each period. We also add a column of weights that are based on the home range confidence estimates' 95% intervals.
...
tapir <-
  ...
  mutate(weight = 1 / (hr_upr_95 - hr_lwr_95), # small CI => large weight
...
```{r hr-gam, eval=TRUE, fig.pos='!h', fig.height=3, fig.cap="Marginal effects of $\\mu(t)$ and $\\sigma^2(t)$ on the tapir's space use (on the log link scale) using a GAM with no shape constraints. The estimated degrees of freedom for each term can be seen in parentheses in the y-axis labels. Shaded areas inticate the 95\\% credible intervals."}
gam(hr_est_95 ~ s(mu, bs = 'ts', k = 5) + s(sigma2, bs = 'ts', k = 5),
    family = Gamma('log'),
    weights = weight, # weights based on CI size
    data = tapir,
    method = 'REML') %>%
  plot(pages = 1, scheme = 1) # trends should be monotonic
```

\clearpage

Due to the small sample size, the spatiotemporal autocorrelation, and the correlation between $\mu(t)$ and $\sigma^2(t)$, it is hard to separate the effects of the two predictors. However, since it is reasonable to expect the effects of $\mu(t)$ and $\sigma^2(t)$ to be monotonic decreaseing and increasing, respectively, we can use a Shape-Constrained Additive Model (SCAM) via the `scam` package [@pya_scam_2022].

```{r hr-scam, eval=TRUE, fig.pos='!h', fig.height=3, fig.cap="Marginal effects of $\\mu(t)$ and $\\sigma^2(t)$ on the tapir's space use (on the log link scale) using a shape-constrained GAM. The estimated degrees of freedom for each term can be seen in parentheses in the y-axis labels. Shaded areas inticate the 95\\% credible intervals."}
m <- scam(hr_est_95 ~
            s(mu, bs = 'mpd', k = 5) +
            s(sigma2, bs = 'mpi', k = 5),
          family = Gamma('log'),
...
This SCAM fits substantially better than a SCAM that only accounts for $\mu(t)$:

```{r, eval=TRUE}
m_mu <- scam(hr_est_95 ~ s(mu, bs = 'mpd', k = 5),
             family = Gamma('log'),
             weights = weight,
             data = tapir)
round(AIC(m, m_mu), 3)
...
We can now create the predictions from the shape-constrained GAM.
...
p_d <- ggplot() +
  ...
  geom_point(aes(mu, hr_est_95), tapir, color = pal[3], alpha = 0.5) +
...
p_e <- ggplot() +
  coord_cartesian(ylim = c(0, 12.5)) +
  geom_point(aes(sigma2, hr_est_95), tapir, color = pal[3], alpha = 0.5) +
