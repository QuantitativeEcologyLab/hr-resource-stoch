...
```{r dag, eval=TRUE, echo=FALSE, fig.cap="Directed Acyclical Graph assumed for inferring the causal effects of $\\e(R)$ and $\\var(R)$ on $H$, where NDVI was used as a proxy for $R$. $Z$ and $S$ indicate confounds that result from habitat-level variables (e.g., competition, predation, etc.) and satellite-level variables (e.g., noise, cloud cover).", fig.height=6, fig.width=6, out.width='75%'}
dagify(NDVI ~ S,
       EN ~ NDVI + E,
       VN ~ NDVI + V,
       E ~ R,
